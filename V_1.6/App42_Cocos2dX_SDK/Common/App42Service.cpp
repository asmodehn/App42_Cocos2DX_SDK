...
namespace App42
{
	...
	void App42Service::populateMetaHeaderParams(std::map<string, string>& metaHeaderParamsMap)
	{
		...
		setACLHeader(metaHeaderParamsMap);
	...
	void App42Service::setACLHeader(std::map<string, string>& metaHeaderParamsMap)
	{
		if (aclList.size() > 0)
		{
			metaHeaderParamsMap["dataACL"] = getJsonStringFromAclList(aclList);
		}
		else if (App42API::getDefaultACL().size() > 0)
		{
			vector<ACL> _aclList = App42API::getDefaultACL();
			metaHeaderParamsMap["dataACL"] = getJsonStringFromAclList(_aclList);
		}
	}

	string App42Service::getJsonStringFromAclList(vector<ACL> &aclObjectArray)
	{
